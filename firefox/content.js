...
3. 重要な単語や表現があれば補足説明してください`
  ...
  function openClaude(prompt) {
    ...
    browser.runtime.sendMessage({ action: 'openClaude', prompt: prompt })
      .then((response) => {
  ...
  // 選択テキストでClaudeを開く（プロンプトインデックス指定）
  function executeClaudeWithSelection(promptIndex = 0) {
    const selection = window.getSelection();
  ...
  document.addEventListener('click', (e) => {
    ...
    const claudeBtn = e.target.closest('.rs-btn-claude');

    if (copyBtn || claudeBtn) {
      const selection = window.getSelection();
      const selectedText = selection.toString().trim();
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (settings.floatingTextAreaEnabled) {
      if (matchesShortcut(e, settings.floatingTextAreaShortcut)) {
        ...
        createFloatingTextArea(selectedText || '', x, y);
        hideFloatingButton();
        return;
      }
    }
